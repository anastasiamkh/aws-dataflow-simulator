...
class CSVtoStream:
    ...
    def load_dataset(self):
        decoded_csv_data = download_file_from_s3(
            bucket_name=self.bucket_name, filepath=self.dataset_filepath
        )

        # parse the file
        file_content = decoded_csv_data.splitlines()

        return file_content

    def start_stream(self) -> None:
        """Convert rows in csv file on AWS S3 to events in AWS Kinesis stream."""
        ...
        for row in csv_reader:
            data = ",".join(row)
            logging.info(f"Creating event form dataset row: {data}")
